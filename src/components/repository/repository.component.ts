...
export class RepositoryComponent extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const { description, forks, language, license, name, stargazers_count, updated_at, html_url: url } = repository;
    ...
    const langClass = language ? language.toLowerCase().replace("++", "-more").replace("#", "-sharp") : "README";

    const $license = document.querySelector<HTMLDivElement>("license-cont");

    const $forks = document.querySelector<HTMLDivElement>("forks-number");

    if (forks && $forks) {
      $forks.innerHTML = this.createForksElement(forks);
    }

    if (license && $license) {
      $license.innerHTML = this.createLicensesElement(license.name);
    }
  ...
  createForksElement(forks: number) {
  ...
  createLicensesElement(license: string) {
