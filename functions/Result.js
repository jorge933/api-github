...
import { StylizeNumbers } from "./OtherFunctions.js";
import { Repos } from "./Repos.js";

export async function Result(user) {
    const $result = document.querySelector('section.container.result');
    const $body = document.querySelector('body')

    const { avatar_url:photo, blog, company, email, location, public_repos, twitter_username } = user;
    
    const photo_html = $result.querySelector('img');
    photo_html.src = photo;
    photo_html.alt = user.login;

    const values = [user.bio, user.followers, user.following, user.login, user.name];
    const html = ['p.bio', 'div.others .followers span.total', 'div.others .following span.total', 'div.names .username', 'div.names .name'];

    for (let i = 0; i < values.length; i++) {
        if (values[i] !== null && values[i].length > 0 || !isNaN(Number(values[i]))) {
            if (!isNaN(Number(values[i]))) {
                $result.querySelector(html[i]).innerText = StylizeNumbers(values[i]);
